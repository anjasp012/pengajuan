            ...
            <div class="card my-4">
                ...
                <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('produk.create') }}" class="btn bg-gradient-success me-3 my-0">Tambah</a>
                    </div>
                </div>
                </div>
                ...
                <div class="table-responsive p-0">
                    ...
                    <tbody>
                        @foreach ($produk as $item => $data)
                            <tr>
                                <td class="align-middle">
                                    <h6 class="mb-0 px-3">{{ $item+1 }}</h6>
                                </td>
                                <td class="align-middle">
                                    <h6 class="mb-0 px-3">{{ $data->nama_produk }}</h6>
                                </td>
                                <td class="align-middle">
                                    <h6 class="mb-0 px-3">{{ $data->projek->nama_projek }}</h6>
                                </td>
                                <td class="align-middle">
                                    <h6 class="mb-0 px-3">{{ $data->tipe->nama_tipe }}</h6>
                                </td>
                                <td class="align-middle">
                                    <h6 class="mb-0 px-3">{{ $data->harga }}</h6>
                                </td>
                                <td class="align-middle">
                                    ...
                                    <ul class="dropdown-menu  dropdown-menu-end  px-2 py-3 me-sm-n5 shadow-lg border" style="z-index: 1;">
                                        <li><a class="dropdown-item" href="{{ route('produk.show', $data->id_produk) }}">Detail</a></li>
                                        <li><a class="dropdown-item" href="{{ route('produk.edit', $data->id_produk) }}">Edit</a></li>
                                        <li>
                                            <form action="{{ route('produk.destroy', $data->id_tipe) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
